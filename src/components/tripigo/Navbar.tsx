...
import { Home, Map, MessageCircle, Users, User, LogIn, Menu, X } from 'lucide-react';
    ...
    <>
      ...
      <nav className="hidden md:flex fixed top-0 left-0 right-0 z-50 glass shadow-sm px-6 py-3 items-center justify-between">
        <Link to="/" className="flex items-center gap-2">
          <img src={tripigoLogo} alt="Tripigo" className="w-8 h-8" />
        ...
        {!user ? (
          <Link to="/auth">
            <Button className="gradient-primary text-primary-foreground border-0 shadow-md hover:shadow-lg transition-shadow">
      ...
      <header className="md:hidden fixed top-0 left-0 right-0 z-50 glass px-4 py-3 flex items-center justify-between">
        <Link to="/" className="flex items-center gap-2">
          <img src={tripigoLogo} alt="Tripigo" className="w-7 h-7" />
          <span className="text-lg font-bold text-gradient">Tripigo</span>
        </Link>
        {!user ? (
          <Link to="/auth">
